...
class CompleteProfileCubit extends Cubit<CompleteProfileState> {
  ...
  Future<void> submit() async {
    ...
    if (!(normalizedPhone.length == 11 && normalizedPhone.startsWith('0'))) {
      emit(state.copyWith(errorMessage: 'Phone number must be 11 digits (e.g. 011xxxxxxxx)'));
      return;
    }
    ...
    try {
      ...
      result.fold(
        ...
        (profileEntity) => emit(state.copyWith(isSubmitting: false, isSuccess: true, profile: profileEntity)),
      );
    } catch (e) {
      emit(
