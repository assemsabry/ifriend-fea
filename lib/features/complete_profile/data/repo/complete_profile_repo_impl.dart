...
class CompleteProfileRepositoryImpl implements CompleteProfileRepository {
  ...
  @override
  Future<Either<ApiErrorHandler, ProfileEntity>> createParentProfile({required String firstName, required String lastName, required String phoneNumber, required String email, File? avatar}) async {
    try {
      final CompleteProfileResponseModel model = await remoteDataSource.createParentProfile(
        firstName: firstName,
        lastName: lastName,
      ...
      if (entity == null) {
        return Left(ApiErrorHandler(message: 'Failed to parse profile response'));
      }
